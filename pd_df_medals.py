...
def avg_medal_count():
    ...
    # YOUR CODE HERE
    bronze_filtered = df['bronze'][df['gold']>0]
    # filtered = df['gold'].map(lambda x: x>0)
    # bronze_filtered = df['bronze'][filtered]

    avg_bronze_at_least_one_gold = numpy.mean(bronze_filtered)
